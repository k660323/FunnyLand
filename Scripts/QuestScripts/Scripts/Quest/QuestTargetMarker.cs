...
public class QuestTargetMarker : MonoBehaviour
{
    ...
    private void Update()
    {
        var rotaion = Quaternion.LookRotation((cameraTransform.position - transform.position).normalized);
        transform.rotation = Quaternion.Euler(0f, rotaion.eulerAngles.y + 180f, 0f);
    }
